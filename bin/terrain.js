...
async function convertAction(source, options) {
  ...
  if (fromLang && toLang && fromLang !== toLang) {
    const msg = `Cannot convert between ${fromFramework} (${fromLang}) and ${toFramework} (${toLang}). Terrain only converts within the same language.`;
    if (jsonOutput) {
      console.log(JSON.stringify({ success: false, error: msg }));
    } else {
      console.error(chalk.red('Error:'), msg);
      console.error(
        chalk.gray(
          'Next steps: Terrain converts within the same language. See `terrain list` for valid directions.'
        )
      );
    }
    process.exit(2);
  ...
  if (isBatch) {
    ...
    const processBatchFile = async (file, fileConverter) => {
      ...
      try {
        ...
        if (onError === 'fail') {
          ...
          const partialContent = `// TERRAIN-WARNING: Conversion incomplete - ${error.message}\n`;
          try {
            await fs.mkdir(path.dirname(outputFilePath), { recursive: true });
    ...
    try {
      ...
      if (onError === 'best-effort') {
        converted = `// TERRAIN-WARNING: Conversion incomplete - ${error.message}\n`;
...
// Hidden from top-level help to keep it readable. Use `terrain shorthands` to see them.
...
program
  ...
  .action(() => {
    ...
    console.log(
      chalk.gray(
        'Usage: terrain convert <source> --from <framework> --to <framework>'
      )
    );
    console.log(chalk.gray('  or:  terrain <shorthand> <source> -o <output>'));
    console.log();
  });
...
program
  ...
  .action(() => {
    ...
    console.log();
    console.log(chalk.gray('Usage: terrain <shorthand> <source> -o <output>'));
    console.log();
  });
...
program
  .command('init')
  .description('Initialize Terrain configuration')
  .option('-f, --force', 'Overwrite existing configuration')
  .action(async (options) => {
    try {
      console.log(chalk.blue('Initializing Terrain configuration...'));

      const configPath = '.terrainrc.json';
  ...
  .action(async (options) => {
    try {
      ...
      if (!(await stateManager.exists())) {
        console.log(
          chalk.yellow(
            'No migration in progress. Run `terrain migrate` to start.'
          )
        );
        return;
      }
...
program
  .command('reset')
  .description('Clear migration state (.terrain/ directory)')
  .option('-d, --dir <path>', 'Project directory', '.')
  ...
  .action(async (options) => {
    try {
      ...
      if (!options.yes) {
        console.log(
          chalk.yellow(
            'This will remove the .terrain/ directory and all migration state.'
          )
        );
...
program
  .command('doctor')
  .description('Run diagnostics and check Terrain setup')
...
program
  .command('serve')
  .description('Start a local API server for the Terrain browser UI')
  ...
  .action(async (options) => {
    try {
      const { TerrainServer } = await import('../src/server/TerrainServer.js');
      const server = new TerrainServer({
        port: parseInt(options.port, 10),
        root: options.root,
      });
      const url = await server.start();
      console.log(chalk.green(`Terrain server listening at ${url}`));
...
program
  .command('ui')
  .description('Open the Terrain browser UI')
  .option('-p, --port <number>', 'Port to listen on (0 = random)', '0')
  .option('--root <path>', 'Project root directory', '.')
  ...
  .action(async (options) => {
    try {
      ...
      const url = await server.start();
      console.log(chalk.green(`Terrain UI: ${url}`));
